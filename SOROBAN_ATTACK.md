...
- **TA-04 auth-arg scope mismatch** — `require_auth_for_args` scoped too narrowly (omitting the recipient/amount) so one authorization is valid for any recipient — the payer's signature is replayed to redirect funds. *Detector (`--scope`): mock the payer's authorization scoped to ONLY `[payer]`, then call `pay(payer, attacker, amount)`; if value actually moves to the attacker, the auth did not bind `(to, amount)` — executed proof. Validated on `scoped_vuln` (500 redirected) vs `scoped_safe` (`require_auth` binds full args → reverts → held), 0 false positives.* **sorohunter: SHIPPED.**
...
- **TM-02 fee-on-transfer accounting** — a vault/bridge credits the deposit `amount` argument instead of measuring its real token balance delta, so a deflationary / fee-on-transfer token makes internal accounting exceed the tokens actually held — insolvency, then a depositor withdraws more than the vault holds. Anchor: **Coinspect Tricorn Bridge TRI-005** (real Soroban mainnet audit). Static linters miss it (dynamic accounting invariant). *Detector (`--feetoken`): deploy a real 10%-fee token + the target vault, deposit through it, compare internal `credit` to tokens held — `credit > held` is the executed proof. Validated on `fee_vault_vuln` (credited 1000, holds 900) vs `fee_vault_safe` (credits the measured delta → held), 0 false positives.* **sorohunter: SHIPPED.**
